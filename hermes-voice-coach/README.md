...
支持单人单机和三人共用设备的英语语音教练。顶部用“Solo｜Group of 3”标签切换，语音、文字和完整记录位于同一段对话中。单人首次录制声音并点击确认，再启用只匹配该声音的降噪与校验；三人先依次登记声音，按人记录讨论，提问后只回答一次。当前根路径为真实应用，`/prototype` 跳转到根路径。完整使用方式与验证边界见[项目说明](../README.md)。
...
- “更多设置”：调整回复参数、声纹匹配门槛，或手动填写尚未出现在目录中的模型。当前声纹接口没有可切换的型号；单人门槛用于服务端强校验，三人门槛同时用于服务端识别和本地归属判断。
...
- 智能体：豆包 ASR + LLM + TTS；应用控制回应时机并同步更正记录。用户曾确认旧版基本单人通话可用；本轮单人声纹版已经过有限的合成声音服务测试，25 人课堂和三人真人声纹效果仍待验证。
